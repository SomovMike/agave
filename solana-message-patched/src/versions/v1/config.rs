...
pub struct TransactionConfig {
    ...
    /// Post-quantum cryptography flag (bit 5).
    /// When true, the transaction carries a Falcon-512 signature trailer.
    /// This is a pure flag — no config value bytes on the wire.
...
impl TransactionConfig {
    ...
    /// Mark this transaction as carrying a PQC (Falcon-512) signature.
    #[must_use]
    pub const fn with_pqc(mut self) -> Self {
        self.pqc = true;
        self
    }

    /// Total size in bytes required to store the config values.
    /// PQC is a pure flag (no config value bytes on the wire).
...
impl TransactionConfigMask {
    ...
    /// PQC (post-quantum cryptography) flag.
    ///
    /// Bit 5. Pure flag — no config value bytes on the wire.
    /// When set, a Falcon-512 signature trailer follows the signatures.
    pub const PQC: u32 = 0b100000;

    /// Mask of all known/supported bits (bits 0-5).
    ...
    pub const fn has_pqc(&self) -> bool {
        (self.0 & Self::PQC) != 0
    }

    /// Total size in bytes required to store the config values.
    /// PQC is a pure flag — it does NOT contribute to config value size.
...
mod tests {
    ...
    fn has_unknown_bits_detects_unsupported_bits() {
        ...
        assert!(!TransactionConfigMask::new(0b100000).has_unknown_bits()); // bit 5 = PQC, now known
        assert!(TransactionConfigMask::new(0x80000000).has_unknown_bits());
        assert!(!TransactionConfigMask::new(0b111111).has_unknown_bits()); // bits 0-5 all known
        assert!(TransactionConfigMask::new(0b1000000).has_unknown_bits()); // bit 6 unknown
    }
